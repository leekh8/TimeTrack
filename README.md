...
### v1.7.0 (2026-07-13) — 시각 위계·다크 대비·모바일 가독성

**개선**
- **타이머 버튼 색 위계 재정리** — `시작`=채운 초록(주 액션) / `일시정지`=중립 슬레이트(비파괴 동작, 위험색 빨강 제거) / `초기화`=경고 주황 / `시간 설정`=외곽선 보조. 주 액션 초록 중복(시작·설정) 해소
- **다크 모드 대비 보정** — 할 일 액션 아이콘(재생·수정·삭제)과 통계·단축키 텍스트가 어두운 배경에 묻히던 문제 수정(`--text-color`가 다크에서 미전환되던 원인)
- **소형폰 가독성** — 루트 폰트 `0.6rem`(≈9.6px)→`0.8rem`, 태블릿 `0.8`→`0.9rem`. 360px에서 가로 오버플로 없음 확인
- 검증: dev 서버 라이브 — 라이트/다크 버튼 위계·아이콘 대비 육안 확인, 360px 오버플로 0, 21개 테스트·빌드 통과
